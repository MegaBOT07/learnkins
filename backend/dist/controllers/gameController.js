...
export const getGames = async (req, res) => {
  try {
    ...
    const games = await Game.find(filter).populate('createdBy', 'name').limit(limit * 1).skip((page - 1) * limit).sort({
      createdAt: -1
    });
...
export const getGame = async (req, res) => {
  try {
    const game = await Game.findById(req.params.id).populate('createdBy', 'name');
    if (!game) {
