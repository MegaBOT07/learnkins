...
export const getOverallStats = async (req, res) => {
  try {
    ...
    const averageProgress = allProgress.length > 0 ? allProgress.reduce((total, p) => total + p.progress, 0) / allProgress.length : 0;

    // Get current streak
    const currentStreak = allProgress.length > 0 ? Math.max(...allProgress.map(p => p.streak.current)) : 0;

    // Get longest streak
    const longestStreak = allProgress.length > 0 ? Math.max(...allProgress.map(p => p.streak.longest)) : 0;
    ...
    try {
      ...
      if (user && user.parentId) {
        ...
        if (control) {
          ...
          let todayLog = control.sessionLogs.find(log => log.date.toDateString() === today);
          if (!todayLog) {
            todayLog = {
              date: new Date(),
              duration: 0,
