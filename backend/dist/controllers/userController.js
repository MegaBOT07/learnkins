...
export const getUsers = async (req, res) => {
  try {
    ...
    const users = await User.find(filter).select('-password').populate('children', 'name email grade').populate('parentId', 'name email').limit(limit * 1).skip((page - 1) * limit).sort({
      createdAt: -1
    });
...
export const getUser = async (req, res) => {
  try {
    ...
    const user = await User.findById(id).select('-password').populate('children', 'name email grade').populate('parentId', 'name email');
    if (!user) {
