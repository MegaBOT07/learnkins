...
export const getShopItems = async (req, res) => {
  try {
    ...
    if (subject && subject !== 'all') filter.$or = [{
      subject
    }, {
      subject: 'all'
    }];
    if (grade && grade !== 'all') filter.$or = [...(filter.$or || []), {
      grade
    }, {
      grade: 'all'
    }];
...
export const purchaseItem = async (req, res) => {
  try {
    ...
    if (existingPurchase) {
      ...
      if (consumables.includes(item.type)) {
        ...
        if (unusedPurchase) {
          return res.status(400).json({
            success: false,
            message: 'You have an unused item of this type. Use it first.'
          });
        }
      }
    }
    ...
    try {
      ...
      await refundTokens(user._id, item.price, 'shop', item._id, `Refund for failed purchase: ${item.title}`).catch(e => console.error('Refund failed:', e));
      console.error('Purchase record creation failed, tokens refunded:', createErr);
      return res.status(500).json({
        success: false,
        message: 'Purchase failed, tokens refunded'
      });
    }
  } catch (err) {
...
export const getAdminShopStats = async (req, res) => {
  try {
    ...
    const totalRevenueAgg = await UserPurchase.aggregate([{
      $group: {
        _id: null,
        total: {
          $sum: '$tokensSpent'
