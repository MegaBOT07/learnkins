...
export const getAdminTokenStats = async (req, res) => {
  try {
    ...
    // Top earners
    const topEarners = await User.find().select('name email tokens level role').sort({
      tokens: -1
    }).limit(10);

    // Recent transactions
    const recentTx = await TokenTransaction.find().sort({
      createdAt: -1
