...
export const getProfessionalQuiz = async (req, res) => {
  try {
    ...
    const quiz = await ProfessionalQuiz.findById(id).populate('createdBy', 'name email');
    if (!quiz) {
...
export const getUserAttempts = async (req, res) => {
  try {
    ...
    const userAttempts = quiz.attempts.filter(attempt => attempt.userId.toString() === userId);
    res.status(200).json({
      success: true,
      data: userAttempts
    });
