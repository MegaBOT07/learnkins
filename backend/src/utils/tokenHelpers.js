...
async function ensureAccount(userId) {
  let account = await TokenAccount.findOne({ userId });
  if (!account) {
...
export async function getDailyStatus(userId) {
  ...
  const lastClaim = account.lastDailyClaim
    ? new Date(account.lastDailyClaim).toISOString().slice(0, 10)
    : null;
  return {
...
export async function getTransactions(userId, limit = 200) {
  return TokenLedgerEntry.find({ userId })
    .sort({ createdAt: -1 })
    .limit(limit);
}

export async function spendTokens(userId, amount, { referenceType, referenceId, reason, description, meta } = {}) {
  if (!userId || amount <= 0) {
  ...
  const existing = await TokenLedgerEntry.findOne({ idempotencyKey });
  if (existing) {
    const account = await ensureAccount(userId);
    return { idempotent: true, balance: account.balance, transaction: existing };
  }
  ...
  return { balance: result.balance, transaction: ledgerEntry };
}

export async function earnTokens(userId, amount, { referenceType, referenceId, reason, description, meta, skipDailyCap } = {}) {
  if (!userId || amount <= 0) {
    throw new Error('Invalid parameters');
  }
...
export async function claimDailyReward(userId) {
  ...
  if (existing) {
    const account = await ensureAccount(userId);
    return { idempotent: true, balance: account.balance, streak: account.currentStreak, tokensEarned: 0 };
  }
  ...
  const result = await TokenAccount.findOneAndUpdate(
    {
      ...
      $expr: {
        $ne: [
          { $ifNull: [{ $dateToString: { format: '%Y-%m-%d', date: '$lastDailyClaim' } }, ''] },
          today,
        ],
      },
    },
  ...
  const prevClaim = result.lastDailyClaim
    ? new Date(result.lastDailyClaim).toISOString().slice(0, 10)
    : null;
  ...
  const ledgerEntry = await TokenLedgerEntry.create({
    ...
    meta: { streak: result.currentStreak, bonus: bonusTokens },
  });
...
export async function refundTokens(userId, amount, originalReferenceType, originalReferenceId, reason) {
  ...
  if (existing) {
    const account = await ensureAccount(userId);
    return { idempotent: true, balance: account.balance };
  }
...
export async function getAdminTokenStats() {
  ...
  const User = (await import('../models/User.js')).default;
  const topEarners = await TokenAccount.find()
    .sort({ lifetimeEarned: -1 })
    .limit(10)
    .populate('userId', 'name email');

  const topBalances = await TokenAccount.find()
    .sort({ balance: -1 })
    .limit(10)
    .populate('userId', 'name email');

  const recentTx = await TokenLedgerEntry.find()
    .sort({ createdAt: -1 })
    .limit(20)
    .populate('userId', 'name email');
  ...
  const dailyActivity = await TokenLedgerEntry.aggregate([
    { $match: { createdAt: { $gte: sevenDaysAgo } } },
    {
      $group: {
        ...
        count: { $sum: 1 },
        tokens: { $sum: { $cond: [{ $eq: ['$type', 'spend'] }, { $multiply: ['$amount', -1] }, '$amount'] } },
      },
    },
    { $sort: { _id: 1 } },
  ]);
  ...
  return {
    ...
    dailyActivity: dailyActivity.map(d => ({ ...d, amount: d.tokens })),
  };
}
