...
const seedTestUsers = async () => {
  try {
    ...
    let demoStudent = await User.findOne({ email: 'student@learnkins.com' });
    if (demoStudent) {
      ...
      demoStudent = await User.create({
        name: 'Demo Student', email: 'student@learnkins.com',
        password: 'student123', role: 'student', grade: '5th',
        parentId: parent._id, isActive: true
      });
    ...
    let demoParent = await User.findOne({ email: 'parent@learnkins.com' });
    if (demoParent) {
      ...
      demoParent = await User.create({
        name: 'Demo Parent', email: 'parent@learnkins.com',
        password: 'parent123', role: 'parent', isActive: true
      });
      console.log('✓ Demo Parent user created');
    }

    // Create/update demo Teacher (teacher@learnkins.com)
    let demoTeacher = await User.findOne({ email: 'teacher@learnkins.com' });
    if (demoTeacher) {
      ...
      demoTeacher = await User.create({
        name: 'Demo Teacher', email: 'teacher@learnkins.com',
        password: 'teacher123', role: 'teacher', isActive: true
      });
