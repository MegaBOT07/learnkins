...
import {
  ...
  getDailyStatus,
  getTransactions as getTransactionsHelper,
  spendTokens,
  earnTokens,
  claimDailyReward as claimDailyHelper,
  getAdminTokenStats,
...
export const awardTokens = async (req, res) => {
  try {
    ...
    if (error.message === 'Daily earn cap reached') {
      return res.status(400).json({ success: false, message: 'Daily earn cap reached' });
    }
...
export const awardUserTokens = async (req, res) => {
  try {
    ...
    const result = await earnTokens(targetUser._id, n, {
      ...
      reason: reason || 'Admin award',
      meta: { awardedBy: req.user._id, ...(meta || {}) },
      skipDailyCap: true,
    });
    ...
    if (error.message === 'Daily earn cap reached') {
      return res.status(400).json({ success: false, message: 'Daily earn cap reached for target user' });
    }
...
export const getEconomyConfig = async (req, res) => {
  try {
    const { getEconomyConfig: getConfig } = await import('../models/EconomyConfig.js');
...
export const updateEconomyConfig = async (req, res) => {
  try {
    ...
    const config = await getConfig();
    const allowedFields = [
      'dailyEarnCap', 'dailyRewardAmount', 'quizMaxTokens', 'quizDailyCount',
      'gameMaxTokens', 'gameDailyCount', 'streakBonusEnabled', 'streakMultiplier',
    ];
