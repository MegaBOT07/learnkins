  ...
  try {
    ...
    if (!payment) {
      ...
      return res.status(200).json({
        ...
        data: { tokensAwarded: existing.tokenAmount, newBalance: balance },
      });
    }
    ...
    const result = await earnTokens(payment.userId, payment.tokenAmount, {
      ...
      meta: { razorpayPaymentId: razorpay_payment_id, plan: payment.plan },
    });
    ...
    if (event.event === 'payment.captured') {
      ...
      if (existing) {
        await earnTokens(existing.userId, existing.tokenAmount, {
          ...
          meta: { razorpayPaymentId: paymentId, source: 'webhook' },
        });
      }
    }
