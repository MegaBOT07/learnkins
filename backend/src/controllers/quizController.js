...
export const getHomepageQuizCards = async (req, res) => {
  try {
    ...
    const allPromise = Quiz.find(baseFilter)
      .select(
        "title description subject chapter grade timeLimit participants difficulty questions attempts.userId"
      )
      .sort({ createdAt: -1 })
      .limit(4);
    ...
    const [allQuizzes, ...categories] = await Promise.all([
      allPromise,
      ...subjectPromises,
    ]);
...
export const getQuizzes = async (req, res) => {
  try {
    ...
    const quizzesWithAttempt = quizzes.map((quiz) => {
      const userAttempt =
        quiz.attempts?.find(
          (attempt) => attempt.userId?.toString() === user_id?.toString()
        ) || null;
    ...
    if (Array.isArray(answers)) {
      answers.forEach((item, index) => {
        const question = quiz.questions.find(
          q => q._id && q._id.toString() === item.questionId
...
export const getLeaderboard = async (req, res) => {
  try {
    ...
    const matchStage = { 'attempts.0': { $exists: true } };
    ...
    const calcPoints = {
      $round: [{
        $multiply: [
          { $divide: [{ $ifNull: ['$totalPoints', 0] }, 100] },
          '$attempts.percentage'
        ]
      }, 0]
    };
    ...
    const results = await Quiz.aggregate([
      { $match: matchStage },
      { $unwind: '$attempts' },
      { $addFields: { pointsEarned: calcPoints } },
      { $group: {
        _id: '$attempts.userId',
        totalPoints: { $sum: '$pointsEarned' },
        totalAttempts: { $sum: 1 },
        lastAttempt: { $max: '$attempts.attemptDate' }
      }},
      { $sort: { totalPoints: -1 } },
      { $limit: 100 },
      { $lookup: {
