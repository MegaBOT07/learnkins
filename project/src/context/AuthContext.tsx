...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Login function
  const login = async (credentials: any): Promise<{ success: boolean; user?: User; error?: string }> => {
    try {
      ...
      if (!error.response) {
        message = 'Network error: Cannot reach the server. Please check if the backend is running on port 5000.';
  ...
  const updateProfile = async (data: any): Promise<{ success: boolean; user?: User; error?: string }> => {
    try {
