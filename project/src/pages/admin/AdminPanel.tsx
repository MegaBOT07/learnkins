  ...
  const fetchAll = async () => {
    ...
    try {
      const [mRes, qRes, uRes, fRes, gRes, sRes, cRes, pRes, siRes, nRes] = await Promise.allSettled([
        materialAPI.getMaterials('all', { limit: 1000 }),
        quizAPI.getQuizzes('all', { limit: 1000 }),
        userAPI.getUsers(),
                                  ...
                                  <div key={idx} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-all">
                                    <div className="flex items-center space-x-4">
                                      <div className={`h-10 w-10 rounded-xl flex items-center justify-center font-bold text-lg
                                        ${tx.type === 'award' || tx.type === 'earn' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}
                                      `}>
                                        {tx.type === 'award' || tx.type === 'earn' ? '+' : '-'}
                                      </div>
                                      <div>
                                        <div className="font-bold text-slate-900 text-sm">{tx.reason || (tx.type === 'award' || tx.type === 'earn' ? 'Reward' : 'Redemption')}</div>
                                    ...
                                    <div className={`text-sm font-black flex items-center gap-1
                                      ${tx.type === 'award' || tx.type === 'earn' ? 'text-emerald-600' : 'text-rose-600'}
                                    `}>
                                      <span>{tx.type === 'award' || tx.type === 'earn' ? '+' : ''}{tx.amount}</span>
